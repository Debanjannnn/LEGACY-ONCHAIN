...
import { Loader2, Clock, Wallet, AlertCircle, User, FileText, Calendar, Coins, Shield, History } from "lucide-react"
...
const CheckMyWill = () => {
  ...
  const { account, connectWallet, getNormalWill, ping, depositNormalWill, withdrawNormalWill, hasCreatedWill } =
    useSmartWill()
  ...
  useEffect(() => {
    async function checkAndFetchWill() {
      ...
      try {
        ...
        setError("Unable to verify will status. Please try again.")
      }
    }
    ...
    return (
      <DotBackground>
        <div className="flex items-center justify-center bg-transparent min-h-screen">
          <Card className="w-full flex flex-col justify-center items-center max-w-md bg-transparent backdrop-blur-sm text-center p-6 pb-9 border border-primary/20">
            <p className="pb-7 text-lg font-medium">Connecting Your Wallet...</p>
            <Loader2 className="w-12 h-12 animate-spin text-primary" />
            <p className="pt-4 text-sm text-muted-foreground">Please approve the connection request in your wallet</p>
          </Card>
        </div>
      </DotBackground>
    )
              ...
              <CardContent className="p-6 space-y-8">
                <div className="grid md:grid-cols-2 gap-6">
                  ...
                  <InfoCard
                    ...
                    content={`${Number(willDetails.amount) / 1e18} Pharos`}
                  />
                  <InfoCard
                    icon={Calendar}
                    title="Created On"
...
const InfoCard = ({ icon: Icon, title, content, highlight = false, className = "" }) => (
  <div
    className={`p-4 rounded-lg transition-all duration-300 hover:shadow-md ${highlight ? "bg-primary text-primary-foreground" : "bg-secondary/80 hover:bg-secondary"}`}
  >
    <h3 className="font-semibold mb-2 flex items-center gap-2">
